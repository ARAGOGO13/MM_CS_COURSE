...
bool command::parse(const char *str) {
    ...
    if (starts_with(p, "quit") && (is_space(p[4]) || p[4] == ';' || p[4] == '\0')) {
    ...
    if (starts_with(p, "insert")) {
        ...
        char name_buf[LEN] = {0};
        int i = 0;
        while (*p && !is_space(*p) && *p != ',' && i < LEN - 1) name_buf[i++] = *p++;
        name_buf[i] = '\0';
        p = skip_spaces(p);
        if (*p != ',') return false;
        p = skip_spaces(p + 1);
    ...
    if (starts_with(p, "where") && is_space(p[5])) {
        ...
        while (*p) {
            ...
            if (op_tmp == operation::none) op_tmp = next_op;
            else if (op_tmp != next_op) return false;
            p = skip_spaces(p);
        }
        c_name = c_name_tmp;
        c_phone = c_phone_tmp;
...
void command::print(FILE *fp) const {
    fprintf(fp, "CMD: type=%d name_cond=%d phone_cond=%d group_cond=%d op=%d "

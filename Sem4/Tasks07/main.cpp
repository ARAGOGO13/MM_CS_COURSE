...
static bool read_config(const char *exe_path, int &k_name, int &k_phone) {
    ...
    if (idx < 2 || values[0] <= 0 || values[1] <= 0) {
        fprintf(stderr, "Config must contain two positive integers (k_name k_phone)\n");
        return false;
    }
    k_name = values[0];
    k_phone = values[1];
...
int main(int argc, char *argv[]) {
    ...
    switch (st) {
        ...
        case io_status::open: fprintf(stderr, "Cannot open file %s\n", argv[1]); return 2;
        case io_status::format: fprintf(stderr, "File format error in %s\n", argv[1]); return 2;
        case io_status::memory: fprintf(stderr, "Memory allocation error\n"); return 2;
        default: fprintf(stderr, "Unknown error reading file\n"); return 2;
    }
    ...
    printf("%s : Result = %d Elapsed = %.2f\n", argv[0], total_found, elapsed_commands);
    // lst.print_stats();
    return 0;
}
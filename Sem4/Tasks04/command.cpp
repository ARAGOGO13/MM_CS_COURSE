...
static bool parse_ordering(const char *&ptr, ordering order[3]) {
    ...
    while (idx < 3) {
        ptr = skip_spaces(ptr);
        if (starts_with(ptr, "name") && (is_space(ptr[4]) || ptr[4] == ',' || ptr[4] == '\0')) {
            order[idx++] = ordering::name;
            ptr += 4;
        } else if (starts_with(ptr, "phone") && (is_space(ptr[5]) || ptr[5] == ',' || ptr[5] == '\0')) {
            order[idx++] = ordering::phone;
            ptr += 5;
        } else if (starts_with(ptr, "group") && (is_space(ptr[5]) || ptr[5] == ',' || ptr[5] == '\0')) {
...
static const char *parse_one_condition(const char *ptr, int &field, condition &cond,
                                       char *value_str, int &value_int, bool &is_like) {
    ptr = skip_spaces(ptr);
    char field_buf[64];
    int i = 0;
    ...
    if (strcmp(field_buf, "name") == 0) field = 0;
    else if (strcmp(field_buf, "phone") == 0) field = 1;
    else if (strcmp(field_buf, "group") == 0) field = 2;
    else return nullptr;

    if (strcmp(op_buf, "like") == 0) cond = condition::like;
    else if (strcmp(op_buf, "not like") == 0) cond = condition::nlike;
    else if (strcmp(op_buf, "=") == 0) cond = condition::eq;
    else if (strcmp(op_buf, "<>") == 0) cond = condition::ne;
    else if (strcmp(op_buf, "<") == 0) cond = condition::lt;
    else if (strcmp(op_buf, ">") == 0) cond = condition::gt;
    else if (strcmp(op_buf, "<=") == 0) cond = condition::le;
    else if (strcmp(op_buf, ">=") == 0) cond = condition::ge;
    else return nullptr;

    if (field == 0) {
        if (cond == condition::like || cond == condition::nlike) {
...
bool command::parse(const char *str) {
    ...
    op = operation::none;
    order[0] = order[1] = order[2] = ordering::none;
    init("", 0, 0);
    ...
    p = skip_spaces(p);
    if (!starts_with(p, "where") || !is_space(p[5])) return false;
    p += 5;
    p = skip_spaces(p);
    if (!*p) return false;
    ...
    condition c_name_tmp = condition::none, c_phone_tmp = condition::none, c_group_tmp = condition::none;
    ...
    while (*p) {
        ...
        int val_int;
        bool is_like;
        const char *next = parse_one_condition(p, field, cond, val_str, val_int, is_like);
...
void command::print(FILE *fp) const {
    fprintf(fp, "CMD: name_cond=%d phone_cond=%d group_cond=%d op=%d ; name='%s' phone=%d group=%d\n",
            (int) c_name, (int) c_phone, (int) c_group, (int) op,

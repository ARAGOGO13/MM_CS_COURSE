...
static bool read_config(const char *exe_path, int &k) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (endptr != p && (*endptr == '\0' || is_space(*endptr))) {
            if (val <= 0) {
                fprintf(stderr, "Invalid k value (must be positive) in config\n");
                fclose(fp);
                return false;
            }
            k = (int) val;
            ...
            fclose(fp);
            fprintf(stderr, "Invalid number format in config\n");
            return false;
        }
    }
    fclose(fp);
    fprintf(stderr, "No valid number found in config\n");
    return false;
}

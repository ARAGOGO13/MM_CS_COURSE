...
int list2::select_and_send(const command &cmd, int fd) const {
...
#define SEND_NODE(node) (node)->print_to_fd(fd, cmd.get_order())
  ...
  if (use_group) {
    if (cmd.get_op() == operation::lor) {
      ...
      for (int i = 0; i < cnt; ++i) SEND_NODE(arr[i]);
      delete[] arr;
      delete[] temp;
      writeToClient(fd, "");
      return cnt;
    ...
    if (cmd.get_op() == operation::lor && has_name_eq && has_phone_eq) {
      ...
      for (int i = 0; i < total; ++i) SEND_NODE(arr[i]);
      delete[] arr;
      delete[] temp;
      writeToClient(fd, "");
      return total;
  ...
  for (list_node *p = head; p; p = p->get_next())
    if (cmd.apply(*p)) arr[idx++] = p;
  if (need_sort) merge_sort(arr, temp, cnt, ob);
  for (int i = 0; i < cnt; ++i) SEND_NODE(arr[i]);
  delete[] arr;
  delete[] temp;
  writeToClient(fd, "");
